...
//
// Pipeline order:
//   1. Validate the request body against monsterFormSchema (server-side re-validation).
//   2. Authenticate the user via Supabase session. user_profile_id always comes
...
//   9. Call Django mark-succeeded with the image metadata (Django will create the
//      MonsterImage row once Step B6 implements the trusted-server boundary).
...
//
// The mark-running / mark-succeeded / mark-failed / mark-blocked Django endpoints
// are 501 stubs until Step B6 implements the trusted-server transition boundary.
// They are called here so the orchestration is fully wired and ready to activate.
// ______________
...
import { Queued_Monster_Image_Gen_Job_Schema } from "@/lib/api/schemas/monster/Monster_Image_Gen_Job_Schema";
 ...
 *
 * These are 501 stubs until Step B6 implements the trusted-server boundary.
 * 501 responses are silently skipped — the orchestration is wired here so it
 * activates automatically once B6 is live. Non-2xx non-501 responses are
 * logged to Sentry as warnings.
 *
...
async function callTransitionEndpoint(
	...
	try {
		const res = await fetchFromDjango(path as unknown as keyof paths, {
			method: "POST",
			headers: {
		...
		if (res.status === 501) {
			// Expected until Step B6 replaces these stubs with real implementations.
			return;
		}
		if (!res.ok) {
...
export async function POST(
	...
	let jobId: string;
	try {
		const jobResponse = await fetchFromDjango("/api/image-generation-jobs/", {
			method: "POST",
			headers: {
				Authorization: `Bearer ${accessToken}`,
				"Content-Type": "application/json",
	...
	//
	// NOTE: Sentry is NOT called here — a blocked prompt is expected content-policy
	// behaviour, not a system failure.
	if (containsBannedTerms(prompt)) {
		await callTransitionEndpoint(
			`/api/image-generation-jobs/${jobId}/mark-blocked/`,
			accessToken,
			{
	...
	if (moderationResult.outcome === "failed") {
		await callTransitionEndpoint(
			`/api/image-generation-jobs/${jobId}/mark-failed/`,
			accessToken,
			{
	...
	await callTransitionEndpoint(
		`/api/image-generation-jobs/${jobId}/mark-running/`,
		accessToken,
	);
	...
	// ── Step 9: Mark job as SUCCEEDED (Django creates MonsterImage in B6) ─────
	//
	// The body here contains everything Django needs to create the MonsterImage row.
	// Once Step B6 implements the trusted-server boundary, this call will atomically:
	//   - Create the MonsterImage with the given ID, URLs, and provider metadata.
	//   - Link it to the Monster (so Monster.image returns it).
	//   - Transition the job to SUCCEEDED and set finished_at.
	//
	// Until B6 is live, the endpoint returns 501 and the image URL is only known
	// to the client via this response — it will not appear in Django's Monster.image.
	await callTransitionEndpoint(
		`/api/image-generation-jobs/${jobId}/mark-succeeded/`,
		accessToken,
		{
			monster_image_id: monsterImageId,
