...
describe("MonsterImageSchema", () => {
	describe("valid inputs", () => {
		it("parses a complete valid image", () => {
			expect(MonsterImageSchema.safeParse(validMonsterImage).success).toBe(true);
		});
	...
	describe("rejects missing required fields", () => {
		...
		it("rejects when provider_model is missing", () => {
			const { provider_model: _model, ...withoutProviderModel } =
				validMonsterImage;
			expect(
				MonsterImageSchema.safeParse(withoutProviderModel).success,
			).toBe(false);
		});

		it("rejects when created_at is missing", () => {
			const { created_at: _created_at, ...withoutCreatedAt } = validMonsterImage;
			expect(MonsterImageSchema.safeParse(withoutCreatedAt).success).toBe(false);
		});
	});
	...
	describe("rejects invalid id", () => {
		...
		it("rejects null id — id is UUID, not nullable", () => {
			expect(
				MonsterImageSchema.safeParse({ ...validMonsterImage, id: null }).success,
			).toBe(false);
		});
	});
