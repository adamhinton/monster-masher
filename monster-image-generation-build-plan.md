...
- [Monster Image Generation Build Plan](#monster-imageGeneration-build-plan)
  ...
  - [Phase 4 Real Image Generation](#phase-4-real-imageGeneration)
    ...
    - [Step 19 Add Real Moderation Implementation](#step-19-add-real-moderation-implementation)
      - [19a Install OpenAI SDK And Confirm Server Env](#19a-install-openai-sdk-and-confirm-server-env)
      - [19b Define Server-Only Moderation Policy](#19b-define-server-only-moderation-policy)
      - [19c Update Moderation Provider Contract For Logging Context](#19c-update-moderation-provider-contract-for-logging-context)
      - [19d Implement Real OpenAI Moderation Call](#19d-implement-real-openai-moderation-call)
      - [19e Evaluate Categories And Scores With App Policy](#19e-evaluate-categories-and-scores-with-app-policy)
      - [19f Add Sentry Moderation Logging](#19f-add-sentry-moderation-logging)
      - [19g Integrate Moderation Before Image Generation](#19g-integrate-moderation-before-image-generation)
      - [19h Normalize Failed And Blocked Moderation Responses](#19h-normalize-failed-and-blocked-moderation-responses)
      - [19i Add Automated Tests](#19i-add-automated-tests)
      - [19j Verification](#19j-verification)
...
Call OpenAI moderation before real image generation, using `omni-moderation-latest`.

This step moderates the final text prompt only.

It does **not** moderate the returned generated image. OpenAI supports image moderation, but returned-image moderation is out of scope for this step.

Important product rule:

Do not blindly reject every `results[0].flagged === true`.

Monster prompts may legitimately trigger `violence`, because monsters can have claws, fangs, combat vibes, scary traits, etc. The app needs its own server-only moderation policy that decides which flagged categories and score thresholds block generation.

#### 19a Install OpenAI SDK And Confirm Server Env

- [] From `frontend/`, install the official OpenAI TypeScript/JavaScript SDK:
...
- [] Confirm `OPENAI_API_KEY` already exists in local env and deployed Vercel env.
- [] Confirm `OPENAI_API_KEY` is server-only.
- [] Do not prefix it with `NEXT_PUBLIC_`.
- [] Do not expose it through client components, Redux, API responses, logs, or Sentry context.
- [] Use the existing server env module if it already validates `OPENAI_API_KEY`.
- [] If `OPENAI_API_KEY` is already validated elsewhere, do not create a duplicate env reader.

Implementation note:

The OpenAI SDK can read `OPENAI_API_KEY` from the server environment automatically when using:

```ts
const openai = new OpenAI();
```

But still keep the key in the project’s server-only env validation so missing config fails clearly.

#### 19b Define Server-Only Moderation Policy

- [] Create or update a server-only moderation policy module.

Suggested file:

```txt
frontend/src/lib/monsterGeneration/imageGeneration/moderation/moderationPolicy.ts
```

- [] Add `import "server-only";` at the top.
- [] Define a strict union of OpenAI moderation category names.

Use the current categories returned by `omni-moderation-latest`:
...
- [] Define a server-only app policy object with:
  - categories that block if OpenAI marks them `true`
  - categories that block if their score exceeds an app-defined threshold
...
- [] Add a comment explaining that these thresholds are project policy, not OpenAI defaults.
- [] Add a comment explaining that `violence` is allowed because monster prompts may naturally mention scary or combat-adjacent traits.
- [] Add a comment explaining that `violence/graphic` is still rejected.
- [] Do not let the client choose categories, thresholds, or policy mode.
- [] Do not accept moderation policy from request JSON, query params, headers, localStorage, cookies, or any client-controlled source.
- [] Do not expose the policy object through a `NEXT_PUBLIC_` module.

Security rule:

The moderation policy must be controlled only by trusted server-side code. Users can submit prompts; users cannot influence how the app interprets moderation results.

#### 19c Update Moderation Provider Contract For Logging Context

The current provider shape is:

```ts
moderate(text: string): Promise<ModerationResult>;
```

Because this step requires Sentry logs with image generation job id and prompt context, update the provider contract to accept a typed input object.
...
export type ModerationInput = {
	...
	generationMode: "fake" | "real";
	route: "/api/monsters/generate-image";
};
```
...
- [] Ensure `imageGenerationJobId` is created server-side.
- [] Do not trust a client-supplied job id as authoritative.
- [] If the backend creates the real generation job before moderation, use that id.
- [] If the backend job does not exist yet, create a server-side `generationRequestId` with `crypto.randomUUID()` and use that consistently in logs and later job creation.
- [] Update `FakeModerationProvider` to accept `ModerationInput`.
- [] Keep `ModerationResult` safe for the client:
...
- [] Do not add raw OpenAI categories or scores to `ModerationResult` if that result can reach client code.

#### 19d Implement Real OpenAI Moderation Call

- [] Implement `OpenAIModerationProvider`.
- [] Keep the implementation server-only.
- [] Import the SDK only in server-only provider code:
...
- [] Call the Moderations API with `omni-moderation-latest`.
- [] Send only the final text prompt.
...
- [] Read `response.results[0]`.
- [] If `results[0]` is missing, treat that as a moderation failure.
- [] Do not use image moderation input in this step.
- [] Do not send the generated image URL, base64 image data, or returned image to moderation in this step.
- [] Do not expose the raw OpenAI response to the browser.

#### 19e Evaluate Categories And Scores With App Policy

- [] Add a helper that evaluates OpenAI’s raw result against `MONSTER_PROMPT_MODERATION_POLICY`.
...
- [] `triggeredCategories` should include rejected categories where `categories[category] === true`.
- [] `triggeredScoreThresholds` should include categories where `category_scores[category] >= policy.scoreThresholds[category]`.
- [] A category listed in `allowFlaggedCategories` should not block merely because its boolean flag is true.
- [] `violence` should be log-only by default.
- [] `violence/graphic` should block.
- [] If both blocked and allowed categories are present, blocked categories win.
- [] Return a generic safe user-facing reason, such as:

```txt
Prompt was blocked by moderation.
```

- [] Do not tell users the exact blocked category or score in the client response.
- [] Keep exact categories and scores in server-side logs/Sentry only.
- [] Add a comment that score thresholds may need recalibration after model upgrades.

Important:

`results[0].flagged` should be logged, but it should not be the only block condition. Use the server-only policy to decide the final app outcome.

#### 19f Add Sentry Moderation Logging

- [] Add Sentry logging for every moderation evaluation in real mode.
- [] Log exactly one Sentry event per moderation evaluation, not one event per category.
- [] Include all category scores in structured Sentry context.
- [] Include category boolean flags in structured Sentry context.
- [] Include the app-level final decision: `allowed`, `blocked`, or `failed`.
- [] Include the OpenAI moderation request id if available.
- [] Include the moderation model returned by OpenAI.
- [] Include the image generation job id or server-side generation request id.
- [] Include route, auth state, generation mode, and environment as tags/context.
- [] Include prompt context carefully.

Recommended prompt logging:
...
- [] Store `promptPreview` as a sanitized and truncated version of the prompt.
- [] Store `promptSha256` so the full prompt can be correlated without always storing raw user text.
- [] Store `promptLength`.
- [] Do not log auth tokens, cookies, Supabase service role key, OpenAI key, or raw provider errors.
- [] Do not include the full raw OpenAI response as a blob.
- [] Do not expose Sentry moderation context to the client.

Optional full-prompt logging:

If full raw prompt logging is truly wanted, gate it behind a private server env flag:

```txt
SENTRY_LOG_FULL_MODERATION_PROMPTS=false
```

Rules:

- [] Default must be `false`.
- [] The flag must be server-only.
- [] Even when enabled, strip obvious HTML/control characters before logging.
- [] Never allow users to enable this from the client.

Suggested Sentry event names:
...
Use `Sentry.withScope()` so moderation tags/context do not leak into unrelated events.

#### 19g Integrate Moderation Before Image Generation

- [] Confirm moderation is called after the local banned-term guard and before the image provider call.
- [] The order in `route.ts` must be:

```txt
validate request
  ↓
build/sanitize final prompt
  ↓
local banned-term guard
  ↓
...
- [] Add comments at each branch point making the order explicit.
- [] If the local banned-term guard blocks the prompt, do not call OpenAI moderation.
- [] If OpenAI moderation blocks the prompt, do not call the image provider.
- [] If OpenAI moderation fails, do not call the image provider.
- [] If fake mode is active, keep using `FakeModerationProvider`.
- [] Real moderation should only be active when `IMAGE_GENERATION_MODE=real`.

#### 19h Normalize Failed And Blocked Moderation Responses

- [] On blocked moderation, return a normal app error response with the project’s standard typed error shape.
- [] Use a safe client message:

```txt
Prompt was blocked by moderation. Try changing the request.
```

- [] Do not expose categories, scores, policy thresholds, OpenAI request ids, or raw provider details to the client.
- [] On OpenAI API/network/SDK errors, return:
...
- [] A moderation failure should fail closed.
- [] Do not silently allow generation if moderation cannot complete.
- [] Treat moderation failure like a recoverable provider failure.
- [] Mark the generation job as failed, not blocked, when the moderation call itself errors.
- [] Use a different internal error code for blocked vs failed:
...
#### 19i Add Automated Tests

Add tests without making real OpenAI calls.

- [] Test allowed prompt proceeds to the image provider.
- [] Test local banned-term prompt stops before OpenAI moderation.
- [] Test local banned-term prompt stops before image provider.
- [] Test fake moderation forced block stops before image provider.
- [] Test real moderation result with only `violence: true` is allowed under the Monster Masher policy.
- [] Test real moderation result with `violence/graphic: true` is blocked.
- [] Test real moderation result with blocked score threshold exceeded is blocked even if category boolean is false.
- [] Test OpenAI SDK/network failure returns `failed`.
- [] Test moderation failure prevents image provider call.
- [] Test blocked moderation response does not expose categories or scores.
- [] Test Sentry logging receives:
  - image generation job id
  - outcome
  - category scores
  - category flags
  - prompt preview/hash/length
- [] Test Sentry logging does not include:
  - OpenAI API key
  - Supabase service role key
  - auth token
  - cookies
  - full raw prompt unless the explicit server-only env flag is enabled
- [] Mock the OpenAI client.
- [] Do not hit the real OpenAI API in tests.
...
- [] `IMAGE_GENERATION_MODE=fake` uses `FakeModerationProvider`.
- [] `IMAGE_GENERATION_MODE=fake` does not call OpenAI.
- [] `IMAGE_GENERATION_MODE=real` calls OpenAI moderation before image generation.
- [] Allowed prompt proceeds.
- [] Prompt with normal monster violence language proceeds.
- [] Prompt with graphic violence is blocked.
- [] Obviously blocked local banned-term prompt stops before OpenAI moderation.
- [] Obviously blocked local banned-term prompt stops before image provider.
- [] Moderation failure becomes a recoverable generation error.
- [] Moderation failure does not call image provider.
- [] No raw moderation response is exposed to the client.
- [] No category scores are exposed to the client.
- [] Sentry has one moderation event per real moderation evaluation.
- [] Sentry event includes job id/request id and all category scores.
- [] Sentry event does not include secrets.
- [] Deployed Vercel env has `OPENAI_API_KEY`.
- [] Production smoke test works with a safe prompt.
- [] Production smoke test blocks a clearly disallowed prompt.
...
Do **not** manually redefine the OpenAI moderation response shape. The OpenAI SDK already exports the moderation result types. Use those as the source of truth, then derive local helper types from them.
...
Still keep the key in the project’s server-only env validation so missing config fails clearly.

#### 19b Use SDK Moderation Types As The Source Of Truth

Update the existing moderation provider file that currently contains:
...
Keep the existing project-owned `ModerationResult` union. That union represents the safe app-level result returned by the provider.

Do **not** redefine OpenAI’s raw moderation response, category map, or score map.
...
Then derive local aliases from the SDK type:

```ts
type OpenAIModerationResult = OpenAI.Moderation;
type OpenAIModerationCategories = OpenAIModerationResult["categories"];
type OpenAIModerationCategoryScores = OpenAIModerationResult["category_scores"];
type OpenAIModerationCategoryAppliedInputTypes =
	OpenAIModerationResult["category_applied_input_types"];

type OpenAIModerationCategory = Extract<
	keyof OpenAIModerationCategories,
	keyof OpenAIModerationCategoryScores
>;
```
...
```ts
const moderationResult: OpenAI.Moderation = moderation.results[0];
```

That gives access to the SDK-defined shape:

- `moderationResult.categories`
- `moderationResult.category_scores`
- `moderationResult.category_applied_input_types`
- `moderationResult.flagged`

Rules:

- [] Do not hand-write a duplicate `Categories` interface.
- [] Do not hand-write a duplicate `CategoryScores` interface.
- [] Do not hand-write a detached string-literal union for all categories.
- [] Category names used in project policy may be written as string literals, but they must be checked with `satisfies readonly OpenAIModerationCategory[]`.
- [] If the OpenAI SDK changes category keys later, TypeScript should complain in the policy file.

#### 19c Define Server-Only Moderation Policy

- [] Create or update a server-only moderation policy section/module.

Suggested file if splitting out:
...
Keep the policy in the existing moderation provider file for now if splitting the file would be premature.

- [] Add `import "server-only";` at the top of any file containing moderation policy.
- [] Define policy categories from the SDK-derived `OpenAIModerationCategory` type.
- [] Do not let the client choose categories, thresholds, or policy mode.
- [] Do not accept moderation policy from request JSON, query params, headers, localStorage, cookies, or any client-controlled source.
- [] Do not expose the policy object through a `NEXT_PUBLIC_` module.
...
```ts
const REJECT_FLAGGED_CATEGORIES = [
	"sexual",
	"sexual/minors",
	"hate",
	"hate/threatening",
	...
	"self-harm/instructions",
	"violence/graphic",
] as const satisfies readonly OpenAIModerationCategory[];

const ALLOW_FLAGGED_CATEGORIES = [
	"violence",
] as const satisfies readonly OpenAIModerationCategory[];
...
export const MONSTER_PROMPT_MODERATION_POLICY = {
	rejectFlaggedCategories: REJECT_FLAGGED_CATEGORIES,
	scoreThresholds: {
		"sexual/minors": 0.01,
		"hate/threatening": 0.1,
		"self-harm/intent": 0.1,
	...
	allowFlaggedCategories: ALLOW_FLAGGED_CATEGORIES,
} as const satisfies ModerationPolicy;
```

Notes:

- [] These thresholds are project policy, not OpenAI defaults.
- [] `violence` is allowed because monster prompts may naturally mention scary or combat-adjacent traits.
- [] `violence/graphic` is still rejected.
- [] Blocked categories win over allowed/log-only categories.
...
#### 19d Update Moderation Provider Contract For Logging Context
...
Update it to accept typed server-side context:
...
- [] Keep the existing `ModerationResult` discriminated union.
- [] Do not add raw OpenAI categories or scores to `ModerationResult` if that result can reach client code.
- [] Ensure `imageGenerationJobId` is created server-side.
- [] Do not trust a client-supplied job id as authoritative.
- [] If the backend creates the real generation job before moderation, use that id.
...
#### 19e Implement Real OpenAI Moderation Call
...
export class OpenAIModerationProvider implements ModerationProvider {
	...
	async moderate(input: ModerationInput): Promise<ModerationResult> {
		try {
			...
			const moderationResult: OpenAI.Moderation | undefined =
				moderation.results[0];

			if (!moderationResult) {
				// Treat malformed/missing provider response as failed closed.
				return {
					outcome: "failed",
					safeErrorMessage: "Moderation check failed.",
				};
			...
			const decision = evaluateMonsterPromptModeration(moderationResult);

			// Sentry logging happens here after evaluation.
			...
			// Sentry failed-event logging happens here.
			return {
				outcome: "failed",
				safeErrorMessage: "Moderation check failed.",
			};
...
- [] Do not send the generated image URL, base64 image data, or returned image to moderation in this step.
- [] Do not expose the raw OpenAI response to the browser.
- [] Do not throw raw OpenAI SDK errors out of the provider.
- [] Fail closed if the moderation call fails.

#### 19f Evaluate Categories And Scores With App Policy

Add a helper that evaluates OpenAI’s SDK-typed result against `MONSTER_PROMPT_MODERATION_POLICY`.
...
```ts
function evaluateMonsterPromptModeration(
	moderation: OpenAI.Moderation,
): ModerationDecision {
	const triggeredCategories =
		MONSTER_PROMPT_MODERATION_POLICY.rejectFlaggedCategories.filter(
			(category) => moderation.categories[category] === true,
		);

	const triggeredScoreThresholds = Object.entries(
		MONSTER_PROMPT_MODERATION_POLICY.scoreThresholds,
	).flatMap(([category, threshold]) => {
		if (threshold === undefined) {
			return [];
		}

		const typedCategory = category as OpenAIModerationCategory;
		const score = moderation.category_scores[typedCategory];

		return score >= threshold ? [typedCategory] : [];
	});

	if (triggeredCategories.length > 0 || triggeredScoreThresholds.length > 0) {
		return {
			outcome: "blocked",
			safeReason: "Prompt was blocked by moderation.",
			triggeredCategories,
...
Avoid `as any`. The `Object.entries()` cast above can be improved with a small typed helper if needed. Do not use `any` to force it through.

Better helper option:
...
```ts
const triggeredScoreThresholds = typedEntries(
	MONSTER_PROMPT_MODERATION_POLICY.scoreThresholds,
).flatMap(([category, threshold]) => {
	if (threshold === undefined) {
		return [];
	...
	const score = moderation.category_scores[category];
	return score >= threshold ? [category] : [];
});
```
...
- [] `violence/graphic` should block.
- [] If both blocked and allowed categories are present, blocked categories win.
- [] Return a generic safe user-facing reason, such as `Prompt was blocked by moderation.`
- [] Do not tell users the exact blocked category or score in the client response.
- [] Keep exact categories and scores in server-side logs/Sentry only.
- [] Add a comment that score thresholds may need recalibration after model upgrades.
...
`moderation.flagged` should be logged, but it should not be the only block condition. Use the server-only policy to decide the final app outcome.

#### 19g Add Sentry Moderation Logging

- [] Add Sentry logging for every moderation evaluation in real mode.
- [] Log exactly one Sentry event per moderation evaluation, not one event per category.
- [] Include all SDK-provided category scores in structured Sentry context.
- [] Include all SDK-provided category boolean flags in structured Sentry context.
- [] Include `category_applied_input_types` in structured Sentry context.
- [] Include the app-level final decision: `allowed`, `blocked`, or `failed`.
- [] Include the OpenAI moderation response id if available from the surrounding response object.
- [] Include the moderation model returned by OpenAI if available from the surrounding response object.
- [] Include the image generation job id or server-side generation request id.
- [] Include route, auth state, generation mode, and environment as tags/context.
- [] Include prompt context carefully.
...
Sentry.withScope((scope) => {
	...
	scope.setContext("moderation", {
		...
		categoryAppliedInputTypes: moderationResult.category_applied_input_types,
	...
	Sentry.captureMessage(`image_generation.moderation.${decision.outcome}`);
});
```
...
#### 19h Integrate Moderation Before Image Generation
...
#### 19i Normalize Failed And Blocked Moderation Responses
...
- [] Mock the OpenAI client.
- [] Do not hit the real OpenAI API in tests.
- [] Test fixtures should use the SDK-shaped `OpenAI.Moderation` type instead of a manually duplicated response type.

Test fixture rule:

If a test needs a fake moderation result, type it as `OpenAI.Moderation`:

```ts
const moderationResult = {
	flagged: true,
	categories: {
		// SDK-shaped category booleans here.
	},
	category_scores: {
		// SDK-shaped category scores here.
	},
	category_applied_input_types: {
		// SDK-shaped applied input types here.
	},
} satisfies OpenAI.Moderation;
```

Do not define a separate fake `ModerationResponse` type.
...
- [] Sentry has one moderation event per real moderation evaluation.
- [] Sentry event includes job id/request id and all category scores.
- [] Sentry event includes `category_applied_input_types`.
- [] Sentry event does not include secrets.
- [] Deployed Vercel env has `OPENAI_API_KEY`.
- [] Production smoke test works with a safe prompt.
- [] Production smoke test blocks a clearly disallowed prompt.
